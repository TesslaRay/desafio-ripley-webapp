...
const Header = () => {
  ...
  if (searchReducer.searchState && searchReducer.searchCounter.length > 0) {
    ...
    for (let i = 0; i < productsReducer.products[0].length; i++) {
      times = times + productsReducer.products[0][i].count;
    }
  }
          ...
          <React.Fragment>
            <div className={classes.items}>
              ...
              searchReducer.searchCounter.length > 0
                ? searchFilter.length
                : productsReducer.products[0].length}
              &nbsp; items
            </div>
            <div className={classes.times}>{times} times</div>
            {!productsReducer.loadingChangeValue && (
